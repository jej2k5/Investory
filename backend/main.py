...
from models import Analysis, User, Watchlist, StockCache
from schemas import (
    AnalysisCreate, AnalysisResponse, AnalysisUpdate,
    WatchlistCreate, WatchlistResponse, WatchlistUpdate,
    StockData, CurrentMetrics, GrowthRates,
    ValuationInput, ValuationOutput,
    MoatEvaluation, AnalysisStats
)
...
def calculate_growth_rates(income_data: List[dict], balance_data: List[dict], 
                          cash_flow_data: List[dict]) -> GrowthRates:
...
def calculate_overall_score(meaning: Optional[int], moat: Optional[int], 
                          management: Optional[int], margin: Optional[int]) -> float:
...
async def get_stock_data(symbol: str, db: Session = Depends(get_db), force_refresh: bool = False):
    ...
    if not force_refresh:
        cached = db.query(StockCache).filter(
...
async def calculate_valuation(valuation: ValuationInput, current_price: float = Query(...)):
    ...
    """
    try:
        sticker = calculate_sticker_price(
...
async def get_analysis_stats(db: Session = Depends(get_db)):
    ...
    ).group_by(Analysis.symbol).order_by(desc('count')).limit(10).all()
    ...
    return AnalysisStats(
        ...
        analyses_by_recommendation=recs_dict,
